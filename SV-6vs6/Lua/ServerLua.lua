...
function calcNpcDropBaseExp(user, npc, damage, killer, dailyextra, membercount)
  ...
  -- 新增条件: 如果玩家等级>=130, 只有怪物等级>135才能获得经验
  if userlv >= 130 and npclv <= 135 then
    local info = string.format("[怪物掉落经验], 玩家等级: %d, 但怪物等级: %d <= 135, 不计算经验", userlv, npclv)
    cPlusLogError(info)
    return 0
  end

  local deltalv = npclv - userlv
